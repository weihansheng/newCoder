...
public class Palindrome2 {
     ...
     * 2、增加一个切点p，将s[i][j]切割为两端s[i][p]、s[p+1][j],则 f[i][j] = f[i][p]+f[p+1][j]+1
     * 所谓的状态转移方程就是上面的式子
     *
    ...
    static public int minCut(String s) {
        ...
        //自低向上求f[i][j], 求f[i][j]需要用到中间的任意f[m][n] m<n&&m>=i&&n<=j
        //第一层循环计算 子字符串的最小切 子字符串长度为k
        for (int k = 0; k <length ; k++) {
            for (int i=0,j=k; j <length ; i++,j++) {
                f[i][j] = compCut(s, i, j, f);
            }
        }
        return f[0][length-1];
    }
    // 利用状态转移方程计算f[i][j]
    static public int compCut(String s,int i,int j,int[][] f) {
        //如果s(i,j)是回文，s(i,j)的最小切为0，即f[i][j]=0
        ...
        int min = s.length();
        for (int p = i; p <j ; p++) {
            //如果s(i,j)不是回文，找切割数最小的两段s(i,p)和s(p,j)，f[i,j]=f(i,p)+f(p,j)+1，加1是因为把s(i,j)分成了两段
            min=Math.min(min,f[i][p]+f[p+1][j]+1);
        }
        return min;
    }
    //判断是否回文串
